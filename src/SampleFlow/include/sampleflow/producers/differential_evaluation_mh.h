    ...
    class DifferentialEvaluationMetropolisHastings : public Producer<OutputType>
    {
      public:
        ...
        sample (const std::vector<OutputType> &starting_points,
                ...
                const std::function<std::pair<OutputType,double> (const OutputType &, const OutputType &, const OutputType &)> &crossover,
          ...
          for (typename std::vector<OutputType>::size_type chain = 0; chain < n_chains; ++chain)
            {
              ...
              if (((crossover_gap == 0)
                  ...
                  (generation > 0))
                {
                  ...
                  const std::pair<OutputType,double> crossover_result = crossover(current_samples[chain], trial_a, trial_b);
                  trial_sample_and_ratio = perturb(crossover_result.first);
                  trial_sample_and_ratio.second *= crossover_result.second;
                }
              else
